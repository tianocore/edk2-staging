  ...
  )
{
  ...
  Status = MesureQemuFwCfgAcpi ((CHAR8 *) Allocate->File, 
  ...
  )
{
  ...
  Status = MesureQemuFwCfgAcpi ("etc/table-loader", 
...
Exit:
  if (EFI_ERROR (Status)) {
    Status = InstallAcpiTablesFromFv(AcpiProtocol);
  }
  return Status;
}
...
EFI_STATUS
EFIAPI
QemuAcpiPlatformEntryPoint (
  IN EFI_HANDLE         ImageHandle,
  IN EFI_SYSTEM_TABLE   *SystemTable
  )
{
  EFI_STATUS Status;
  Status = AcpiPlatformEntryPoint(ImageHandle, SystemTable, InstallAcpiTablesFromQemu);
  return Status;
}
